...
class EndToEndM1Test {
  ...
  fun testEndToEndFlow() {
    ...
    // Thread.sleep(1000)
    composeTestRule.onNodeWithTag("addFoodItemTitle").performScrollTo().assertIsDisplayed()
    composeTestRule.onNodeWithTag("inputFoodName").performScrollTo().performTextInput("Apple")
    composeTestRule.onNodeWithTag("inputFoodAmount").performScrollTo().performTextInput("5")
    composeTestRule.onNodeWithTag("inputFoodExpireDate").performScrollTo().performTextClearance()
    composeTestRule.onNodeWithTag("inputFoodExpireDate").performTextInput("29102025")
    composeTestRule.onNodeWithTag("inputFoodOpenDate").performScrollTo().performTextClearance()
    ...
    // User inputs incorrect data and inputs the correct data again
    composeTestRule.onNodeWithText("Expire Date cannot be before Buy Date").performScrollTo().assertIsDisplayed()
    // Thread.sleep(1000)
    // Thread.sleep(1000)
    composeTestRule.onNodeWithTag("inputFoodExpireDate").performScrollTo().performTextClearance()
