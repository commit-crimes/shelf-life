...
fun HouseHoldCreationScreen(
    ...
    LaunchedEffect(householdToEdit) {
        householdToEdit?.let {
            householdViewModel.selectHouseholdToEdit(it)
        }
    }
            ...
            Column(
                ...
                memberEmailList.forEachIndexed { index, email ->
                    Row(
                        ...
                        IconButton(
                            onClick = {
                                memberEmailList.removeAt(index)
                            },
                        ...
                        if (householdViewModel.checkIfHouseholdNameExists(houseHoldName) &&
                            ...
                            coroutineScope.launch {
                                householdViewModel.getUserIdsByEmails(memberEmailList) { emailToUid ->
                                    val missingEmails = memberEmailList.filter { it !in emailToUid.keys }
                                    ...
                                    if (householdToEdit != null) {
                                        val updatedHouseHold = householdToEdit!!.copy(
                                            name = houseHoldName,
                                            members = memberUids
                                        )
