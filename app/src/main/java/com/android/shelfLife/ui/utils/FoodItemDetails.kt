...
fun FoodItemDetails(foodItem: FoodItem) {
    ...
    val formattedExpiryDate = foodItem.expiryDate?.let { formatTimestampToDate(it) } ?: "No Expiry Date"
    ...
    ElevatedCard(
        ...
        Column(modifier = Modifier.padding(16.dp)) {
            FoodItemDetailText("Category: ${foodItem.foodFacts.category.name}", "categoryText", textStyle)
            ...
            FoodItemDetailText("Energy: ${foodItem.foodFacts.nutritionFacts.energyKcal} Kcal", "energyText", textStyle)
            FoodItemDetailText("Proteins: ${foodItem.foodFacts.nutritionFacts.proteins} g", "proteinsText", textStyle)
            FoodItemDetailText("Quantity: ${foodItem.foodFacts.quantity.amount} ${foodItem.foodFacts.quantity.unit}", "quantityText", textStyle)
        }
    }
}
