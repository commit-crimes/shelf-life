...
class InvitationRepositoryFirestoreTest {
    ...
    @Test
    fun getInvitationsReturnsListOfInvitations() : Unit = runBlocking {
        val invitationIds = listOf("inv1", "inv2")
        val invitations = invitationRepository.getInvitations(invitationIds)

        assertEquals(1, invitations.size)
        assertEquals("mockInvitationId", invitations[0].invitationId)
    }

    @Test
    fun getInvitationsReturnsEmptyListWhenNoIdsProvided() = runBlocking {
        val invitationIds = emptyList<String>()
        val invitations = invitationRepository.getInvitations(invitationIds)
        assertEquals("Should return empty list", 0, invitations.size)
    }
