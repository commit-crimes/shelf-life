...
class HouseholdRepositoryFirestoreTest {
  ...
  fun initializeHouseholdsWithEmptyListSetsEmptyCache(): Unit = runBlocking {
    ...
    fun selectHouseholdSetsSelectedHousehold() = runBlocking {
        ...
        val household = HouseHold("house999", "Selected Household", listOf("userX"), listOf("recipeZ"), emptyMap(), emptyMap())
    ...
    fun selectHouseholdToEditSetsHouseholdToEdit() = runBlocking {
        ...
        val household = HouseHold("houseEdit", "Editing Household", listOf("userX"), listOf("recipeZ"), emptyMap(), emptyMap())
    ...
    fun updateStinkyPointsErrorDoesNotChangeSelectedHousehold() = runBlocking {
        ...
        `when`(mockDocument.update("stinkyPoints", newStinky)).thenThrow(RuntimeException("Stinky Error"))
        ...
        assertNotNull("Selected household should not be null", updatedHousehold,)
        assertTrue("Stinky points should not change on error",
    ...
    fun startListeningForHouseholdsWithEmptyListClearsHouseholds() = runBlocking {
        ...
        val field = HouseholdRepositoryFirestore::class.java.getDeclaredField("_households")
        field.isAccessible = true
        field.set(houseHoldRepository, kotlinx.coroutines.flow.MutableStateFlow(listOf(
            HouseHold("id1","test",listOf(), listOf(), emptyMap(), emptyMap())
        )))
    ...
    fun addHouseholdErrorRollsBackChanges() = runBlocking {
        val household = HouseHold("houseErr","Error Household", listOf("u1"), listOf("r1"), emptyMap(), emptyMap())
    ...
    fun getHouseholdMembersReturnsCorrectMembers() = runBlocking {
        val household = HouseHold("houseM","Name", listOf("member1","member2"), listOf(), emptyMap(), emptyMap())
    ...
    fun getHouseholdMembersReturnsEmptyIfNotFound() = runBlocking {
        val fieldH = HouseholdRepositoryFirestore::class.java.getDeclaredField("_households")
        fieldH.isAccessible = true
        fieldH.set(houseHoldRepository, kotlinx.coroutines.flow.MutableStateFlow(emptyList<HouseHold>()))
