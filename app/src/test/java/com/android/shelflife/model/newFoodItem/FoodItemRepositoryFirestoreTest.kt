...
class FoodItemRepositoryFirestoreTest {
    ...
    fun `startListeningForFoodItems updates local cache on snapshot changes`() {
        val listenerCaptor = ArgumentCaptor.forClass(EventListener::class.java as Class<EventListener<QuerySnapshot>>)
        val registrationMock = mock(ListenerRegistration::class.java)

        `when`(mockSubCollection.addSnapshotListener(listenerCaptor.capture())).thenReturn(registrationMock)
