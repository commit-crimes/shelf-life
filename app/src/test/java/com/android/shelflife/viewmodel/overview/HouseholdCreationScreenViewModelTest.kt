...
class HouseholdCreationScreenViewModelTest {
    ...
    fun `init with householdToEdit sets emailList from members`() = runTest {
        ...
        val household = HouseHold("h1", "MyHouse", listOf("uid1","uid2"), emptyList(), emptyMap(), emptyMap())
        householdToEditFlow.value = household

        `when`(houseHoldRepository.getHouseholdMembers("h1")).thenReturn(listOf("uid1","uid2"))
        `when`(userRepository.getUserEmails(listOf("uid1","uid2"))).thenReturn(
            mapOf("uid1" to "email1@test.com", "uid2" to "email2@test.com")
        )

        viewModel = HouseholdCreationScreenViewModel(houseHoldRepository, foodItemRepository, invitationRepository, userRepository)
        advanceUntilIdle()
    ...
    @Test
    fun `confirmHouseholdActions creates new household if no householdToEdit and name valid`() = runTest {
        viewModel = HouseholdCreationScreenViewModel(
            houseHoldRepository, foodItemRepository, invitationRepository, userRepository
        )
        val user = User(
            "uid", "Tester", "tester@mail.com", "",
    ...
    fun `confirmHouseholdActions editing household with same name doesn't fail`() = runTest {
        ...
        val oldHousehold = HouseHold("hid","OldName",listOf("u1"),emptyList(),emptyMap(),emptyMap())
    ...
    fun `confirmHouseholdActions editing household with new name and adding members`() = runTest {
        ...
        `when`(userRepository.getUserIds(setOf("new@test.com"))).thenReturn(mapOf("new@test.com" to "u2"))
    ...
    fun `confirmHouseholdActions editing household and removing members`() = runTest {
        ...
        val oldHousehold = HouseHold("hid","OldName",listOf("u1","u2"), emptyList(), emptyMap(), emptyMap())
        householdToEditFlow.value = oldHousehold
    ...
    @Test
    fun `confirmHouseholdActions fails if user not logged in when creating new household`() = runTest {
        viewModel = HouseholdCreationScreenViewModel(
            houseHoldRepository, foodItemRepository, invitationRepository, userRepository
        )
        userFlow.value = null
    ...
    fun `updateHousehold selects and fetches items if current selected matches`() = runTest {
        ...
        `when`(userRepository.getUserIds(setOf("friend@mail.com"))).thenReturn(mapOf("friend@mail.com" to "fId"))
