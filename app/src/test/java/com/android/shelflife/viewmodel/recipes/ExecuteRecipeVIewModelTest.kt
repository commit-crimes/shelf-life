...
class ExecuteRecipeViewModelTest {
    ...
    fun setUp() {
        ...
        val tomatoItem = FoodItem(
            uid = "f1",
            owner = "currentUserUID",
            foodFacts = FoodFacts(name = "Tomato", quantity = Quantity(amount = 5.0, unit = FoodUnit.COUNT))
        )
        val cheeseItem = FoodItem(
            uid = "f2",
            owner = "otherUserUID",
            foodFacts = FoodFacts(name = "Cheese", quantity = Quantity(amount = 3.0, unit = FoodUnit.COUNT))
        )
        foodItemsFlow.value = listOf(tomatoItem, cheeseItem)

        viewModel = ExecuteRecipeViewModel(houseHoldRepository, foodItemRepository, recipeRepository, userRepository)
