...
class DeletionConfirmationViewModelTest {
  ...
  fun setUp() {
    ...
    whenever(houseHoldRepository.updateHousehold(any(), any<(String) -> Unit>())).thenAnswer {
      ...
      householdsFlow.value =
          householdsFlow.value.map { hh ->
    ...
    viewModel =
        DeletionConfirmationViewModel(houseHoldRepository, userRepository, foodItemRepository)
  }
  ...
  fun `deleteHouseholdById household with multiple members updates household`() = runTest {
    ...
    verify(houseHoldRepository)
        .updateHousehold(
            argThat { this.uid == householdId && !this.members.contains("currentUserUID") }, any())
  ...
  fun `deleteHouseholdById household with single member deletes household`() = runTest {
    ...
    // After callback, userRepository.deleteHouseholdUID and
    // foodItemRepository.deleteHouseholdDocument should be called
  ...
  @Test
  fun `deleteHouseholdById when deleting selected household and no next household remains`() =
      runTest {
        val h1 =
            HouseHold(
                uid = "h1",
        ...
        // The ViewModel code calls selectHousehold with householdToSelect, which should be null
        // now.
  ...
  @Test
  fun `deleteHouseholdById when deleting a non-selected household does not change selected`() =
      runTest {
        val h1 =
            HouseHold(
                uid = "h1",
        ...
        // h2 had multiple members, so it should be updated, and callback triggers
        // userRepository.deleteHouseholdUID
        verify(houseHoldRepository)
            .updateHousehold(
                argThat { this.uid == "h2" && !this.members.contains("currentUserUID") }, any())
